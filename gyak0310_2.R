...
#Cauchy feladat
vec<-xi
cauchy<-nlm(cauchy_loglik,parm<-c(median(xi),mad(xi)*qnorm(0.75)),
...
while(abs(T-T2)>eps){
T=T2;
Tw=T+s*mean(psi((eta-T)/s));s=s*sqrt(12*mean(psi((eta-T)/s)^2));
i=i+1;
}
